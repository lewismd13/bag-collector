...
function undelay(macro) {
  if (macro instanceof combat_Macro) return macro;else return macro();
}
/**
...
var CombatStrategy = /*#__PURE__*/function () {
  ...
  dist_combat_createClass(CombatStrategy, [{
    ...
    value: function compile(resources, defaults, location) {
      ...
      var result = new combat_Macro(); // If there is macro precursor, do it now
      ...
      this.macros.forEach((value, key) => {
        ...
        monster_macros.add(key, (_Macro = new combat_Macro()).step.apply(_Macro, dist_combat_toConsumableArray(value.map(undelay))));
      });
      ...
      this.actions.forEach((action, key) => {
        ...
        if (macro) monster_actions.add(key, new combat_Macro().step(macro));
      });
    ...
    value: function compileAutoattack() {
      var result = new combat_Macro(); // Perform any monster-specific autoattacks (these may or may not end the fight)

      var monster_macros = new CompressedMacro();
      this.autoattacks.forEach((value, key) => {
        var _Macro2;

        monster_macros.add(key, (_Macro2 = new combat_Macro()).step.apply(_Macro2, dist_combat_toConsumableArray(value.map(undelay))));
      });
      result.step(monster_macros.compile()); // Perform the non-monster specific macro
...
var CombatResources = /*#__PURE__*/function () {
  ...
  dist_combat_createClass(CombatResources, [{
    ...
    value: function getMacro(action) {
      ...
      if (resource.do instanceof external_kolmafia_namespaceObject.Item) return new combat_Macro().item(resource.do);
      if (resource.do instanceof external_kolmafia_namespaceObject.Skill) return new combat_Macro().skill(resource.do);
      return resource.do;
    }
  }]);
...
var effectResources = [{
  ...
  prepare: () => {
    if (!have(template_string_$skill(effects_templateObject7 || (effects_templateObject7 = effects_taggedTemplateLiteral(["Seek out a Bird"]))))) {
  ...
  do: () => (0,external_kolmafia_namespaceObject.useSkill)(template_string_$skill(effects_templateObject9 || (effects_templateObject9 = effects_taggedTemplateLiteral(["Visit your Favorite Bird"]))))
}, {
  name: "Bird",
  ...
  prepare: () => {
    if (!have(template_string_$skill(effects_templateObject12 || (effects_templateObject12 = effects_taggedTemplateLiteral(["Seek out a Bird"]))))) {
  ...
  do: () => (0,external_kolmafia_namespaceObject.useSkill)(template_string_$skill(effects_templateObject14 || (effects_templateObject14 = effects_taggedTemplateLiteral(["Seek out a Bird"]))), 6 - property_get("_birdsSoughtToday"))
}, {
  name: "Pool Table",
...
function acquire(quantity, item, maxPrice) {
  ...
  debug("Trying to acquire ".concat(formatAmountOfItem(quantity, item), " with a max price of ").concat(formatNumber(maxPrice)));
...
var farmingPotions = external_kolmafia_namespaceObject.Item.all().filter(item => item.tradeable && !blacklist.includes(item) && (0,external_kolmafia_namespaceObject.itemType)(item) === "potion").map(item => new Potion(item)).filter(potion => potion.netValue() > 0).sort((a, b) => b.netValue() - a.netValue());
function setupPotions() {
  var excludedEffects = new Set(getActiveEffects().map(effect => getMutuallyExclusiveEffectsOf(effect)).flat());

  var _iterator2 = potions_createForOfIteratorHelper(farmingPotions),
      _step2;
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      var desiredAmount = Math.floor(args.turns / potion.effectDuration());
      var acquiredAmount = acquire(desiredAmount, potion.item, potion.netValue());
      debug("Using ".concat(formatAmountOfItem(acquiredAmount, potion.item)));
      var used = (0,external_kolmafia_namespaceObject.use)(acquiredAmount, potion.item);
...
function bubbleVision() {
  if (have($effect(potions_templateObject5 || (potions_templateObject5 = potions_taggedTemplateLiteral(["Bubble Vision"]))))) return;
  var potion = new Potion(template_string_$item(potions_templateObject6 || (potions_templateObject6 = potions_taggedTemplateLiteral(["bottle of bubbles"]))), {
    itemDrop: 50
  });
...
var Potion = /*#__PURE__*/function () {
  ...
  potions_createClass(Potion, [{
    ...
    value: function itemDrop() {
      ...
      (0,external_kolmafia_namespaceObject.setLocation)($location(potions_templateObject7 || (potions_templateObject7 = potions_taggedTemplateLiteral(["none"]))));
    ...
    value: function grossValue() {
      var duration = Math.min(this.effectDuration(), turnsRemaining());
...
var Flower = /*#__PURE__*/function () {
  ...
  Florist_createClass(Flower, [{
    ...
    value: function dig() {
      ...
      (0,external_kolmafia_namespaceObject.visitUrl)("whichchoice=720&choice.php?option=2&plnti=".concat(plantNumber, "&pwd"));
    ...
    value: function plant() {
      ...
      (0,external_kolmafia_namespaceObject.visitUrl)("choice.php?whichchoice=720&option=1&plant=".concat(this.id, "&pwd"));
      return this.isPlantedHere();
    }
  }], [{
...
var engine_combat_templateObject, engine_combat_templateObject2, combat_templateObject3, combat_templateObject4, combat_templateObject5;
...
var myActions = ["kill", "banish", "abort"];
...
var MyActionDefaults = /*#__PURE__*/(/* unused pure expression or super */ null && (function () {
  ...
  engine_combat_createClass(MyActionDefaults, [{
    ...
    value: function delevel() {
      return new Macro().skill($skill(engine_combat_templateObject || (engine_combat_templateObject = engine_combat_taggedTemplateLiteral(["Curse of Weaksauce"])))).trySkill($skill(engine_combat_templateObject2 || (engine_combat_templateObject2 = engine_combat_taggedTemplateLiteral(["Micrometeorite"])))).tryItem($item(combat_templateObject3 || (combat_templateObject3 = engine_combat_taggedTemplateLiteral(["Rain-Doh indigo cup"])))).trySkill($skill(combat_templateObject4 || (combat_templateObject4 = engine_combat_taggedTemplateLiteral(["Summon Love Mosquito"])))).tryItem($item(combat_templateObject5 || (combat_templateObject5 = engine_combat_taggedTemplateLiteral(["Time-Spinner"]))));
    }
  }]);
...
function runwaySource() {
  return have(template_string_$item(tasks_templateObject || (tasks_templateObject = tasks_taggedTemplateLiteral(["Greatest American Pants"])))) ? template_string_$item(tasks_templateObject2 || (tasks_templateObject2 = tasks_taggedTemplateLiteral(["Greatest American Pants"]))) : template_string_$item(tasks_templateObject3 || (tasks_templateObject3 = tasks_taggedTemplateLiteral(["navel ring of navel gazing"])));
}
function coldMedicineCabinet() {
  if ((0,external_kolmafia_namespaceObject.getWorkshed)() !== template_string_$item(tasks_templateObject4 || (tasks_templateObject4 = tasks_taggedTemplateLiteral(["cold medicine cabinet"])))) return;
  ...
  if ((0,external_kolmafia_namespaceObject.expectedColdMedicineCabinet)()["pill"] !== template_string_$item(tasks_templateObject5 || (tasks_templateObject5 = tasks_taggedTemplateLiteral(["Extrovermectin\u2122"])))) return;
...
function floristFriar() {
  ...
  if ((0,external_kolmafia_namespaceObject.myLocation)() !== $location(tasks_templateObject6 || (tasks_templateObject6 = tasks_taggedTemplateLiteral(["The Neverending Party"])))) return;
  if (isFull()) return;
...
var BaggoQuest = {
  ...
  tasks: [{
    ...
    do: () => {
      (0,external_kolmafia_namespaceObject.visitUrl)((0,external_kolmafia_namespaceObject.toUrl)($location(tasks_templateObject7 || (tasks_templateObject7 = tasks_taggedTemplateLiteral(["The Neverending Party"])))));
    ...
    do: $location(tasks_templateObject8 || (tasks_templateObject8 = tasks_taggedTemplateLiteral(["The Neverending Party"]))),
    post: () => {
      coldMedicineCabinet();
      floristFriar();
    },
    outfit: () => {
      return {
        weapon: template_string_$item(tasks_templateObject9 || (tasks_templateObject9 = tasks_taggedTemplateLiteral(["June cleaver"]))),
        offhand: template_string_$item(tasks_templateObject10 || (tasks_templateObject10 = tasks_taggedTemplateLiteral(["carnivorous potted plant"]))),
        acc1: template_string_$item(tasks_templateObject11 || (tasks_templateObject11 = tasks_taggedTemplateLiteral(["mafia thumb ring"]))),
        acc2: (0,external_kolmafia_namespaceObject.myClass)().primestat !== $stat(tasks_templateObject12 || (tasks_templateObject12 = tasks_taggedTemplateLiteral(["moxie"]))) ? template_string_$item(tasks_templateObject13 || (tasks_templateObject13 = tasks_taggedTemplateLiteral(["mime army infiltration glove"]))) : undefined,
        familiar: template_string_$familiar(tasks_templateObject14 || (tasks_templateObject14 = tasks_taggedTemplateLiteral(["Reagnimated Gnome"]))),
        famequip: template_string_$item(tasks_templateObject15 || (tasks_templateObject15 = tasks_taggedTemplateLiteral(["gnomish housemaid's kgnee"]))),
        equip: [runwaySource()],
        modifier: "item"
      };
    },
    effects: [template_string_$skill(tasks_templateObject16 || (tasks_templateObject16 = tasks_taggedTemplateLiteral(["Blood Bond"]))), template_string_$skill(tasks_templateObject17 || (tasks_templateObject17 = tasks_taggedTemplateLiteral(["Leash of Linguini"]))), template_string_$skill(tasks_templateObject18 || (tasks_templateObject18 = tasks_taggedTemplateLiteral(["Empathy of the Newt"]))), template_string_$skill(tasks_templateObject19 || (tasks_templateObject19 = tasks_taggedTemplateLiteral(["The Spirit of Taking"]))), template_string_$skill(tasks_templateObject20 || (tasks_templateObject20 = tasks_taggedTemplateLiteral(["Fat Leon's Phat Loot Lyric"]))), template_string_$skill(tasks_templateObject21 || (tasks_templateObject21 = tasks_taggedTemplateLiteral(["Singer's Faithful Ocelot"])))].filter(skill => have(skill)).map(skill => (0,external_kolmafia_namespaceObject.toEffect)(skill)),
    choices: {
      1324: 5
    },
    combat: (_CombatStrategy$banis = (_CombatStrategy$banis2 = (_CombatStrategy = new combat_CombatStrategy()).banish.apply(_CombatStrategy, tasks_toConsumableArray($monsters(tasks_templateObject22 || (tasks_templateObject22 = tasks_taggedTemplateLiteral(["biker, party girl, \"plain\" girl"])))))).macro.apply(_CombatStrategy$banis2, [combat_Macro.step("pickpocket").if_([template_string_$item(tasks_templateObject23 || (tasks_templateObject23 = tasks_taggedTemplateLiteral(["van key"]))), template_string_$item(tasks_templateObject24 || (tasks_templateObject24 = tasks_taggedTemplateLiteral(["unremarkable duffel bag"])))].map(item => "match \"".concat(item, "\"")).join(" || "), combat_Macro.runaway())].concat(tasks_toConsumableArray($monsters(tasks_templateObject25 || (tasks_templateObject25 = tasks_taggedTemplateLiteral(["burnout, jock"]))))))).kill.apply(_CombatStrategy$banis, tasks_toConsumableArray($monsters(tasks_templateObject26 || (tasks_templateObject26 = tasks_taggedTemplateLiteral(["burnout, jock"])))))
  }]
};
